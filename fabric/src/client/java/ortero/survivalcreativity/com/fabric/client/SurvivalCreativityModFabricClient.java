package ortero.survivalcreativity.com.fabric.client;
...
public class SurvivalCreativityModFabricClient implements ClientModInitializer {
	@Override
	public void onInitializeClient() {
		ModKeybindsFabric.register();

		UseBlockCallback.EVENT.register((player, level, hand, hitResult) -> {
			return ImaginationInteractions.onUseBlock(player, level, hand, hitResult);
		});
		ClientPlayerBlockBreakEvents.AFTER.register((level, player, pos, state) -> {
			ImaginationInteractions.onBlockBroken(level, player, pos, state);
		});

		ClientPlayNetworking.registerGlobalReceiver(ShareImaginationS2CPayload.TYPE, (payload, context) -> {
			ImaginationShare.handleLargeShareReceived(
				payload.shareId(),
				payload.senderName(),
		...
		ClientTickEvents.END_CLIENT_TICK.register(client -> {
			...
			while (ModKeybindsFabric.TOGGLE_EDIT.consumeClick()) {
				ImaginationManager.INSTANCE.toggleEdit(client);
			}
			while (ModKeybindsFabric.SAVE.consumeClick()) {
				ImaginationManager.INSTANCE.requestSave(client);
			}
			while (ModKeybindsFabric.OPEN_MENU.consumeClick()) {
				ImaginationManager.INSTANCE.openMenu(client);
			}
			while (ModKeybindsFabric.HIDE_PREVIEW.consumeClick()) {
